function Get-CIPPManagedDevicesReport {
    <#
    .SYNOPSIS
        Returns managed devices from the CIPP reporting database
    ...
    if ($TenantFilter -eq 'AllTenants') {
        ...
        foreach ($Tenant in $Tenants) {
            try {
                ...
                Write-LogMessage -API 'ManagedDevicesReport' -tenant $Tenant `
    ...
    $Items = Get-CIPPDbItem -TenantFilter $TenantFilter -Type 'ManagedDevices' |
        Where-Object { $_.RowKey -notlike '*-Count' }
